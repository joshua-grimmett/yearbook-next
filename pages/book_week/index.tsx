...
import bookWeek from '../../public/other/book_week/data'
        ...
        <div className="grid w-[179mm] gap-[5mm] top-[50mm] absolute">
          <div className="relative">
            <div className="grid grid-cols-3 gap-[3mm]">
              {props.images.slice(10).map((image: any, key: number) => (
                <Photo photo={image} height="41.8mm" width="100%" key={key}/>
              ))}
            </div>
          </div>
        </div>
...
const CouncilTemplate: NextPage = () => {
  return (
    <Layout title={bookWeek.title}>
      <div className={`page ${bookWeek.colours}`}>
        <LeftPage {...bookWeek}/>
        <RightPage {...bookWeek}/>
      </div>
    </Layout>
